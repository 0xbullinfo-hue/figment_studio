...
const ClientDashboard: React.FC = () => {
  ...
  const { projects, auth, invoices } = useStudioStore();
  ...
  const totalPending = useMemo(() => pendingInvoices.reduce((sum, i) => sum + i.amount, 0), [pendingInvoices]);
  ...
  return (
    <DashboardShell title="Overview" subtitle={`${activeProjects.length} active projects`}>
      <Helmet>
        ...
        <div className="max-w-6xl mx-auto space-y-10">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/5 pb-8 gap-6">
            <div className="text-left">
              ...
              <p className="text-white/40 text-sm mt-1 font-sans">
                You have {activeProjects.length} active project{activeProjects.length !== 1 ? 's' : ''} and {pendingInvoices.length} pending invoice{pendingInvoices.length !== 1 ? 's' : ''}.
              </p>
            </div>
            <div className="flex items-center gap-3">
              <button
                onClick={() => navigate('/estimate')}
                className="px-5 py-2.5 bg-primary hover:bg-primary-hover text-white rounded-xl font-bold text-xs uppercase tracking-wider shadow-lg shadow-primary/20 active:scale-95 transition-all flex items-center gap-2"
              >
              ...
              <button
                ...
                className="flex items-center gap-3 bg-[#161616] border border-white/10 px-5 py-2.5 rounded-xl hover:border-primary/40 transition-all text-left group cursor-pointer"
              >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Completed', value: completedProjects.length, icon: CheckCircle2 },
              { label: 'Total Invoices', value: (invoices || []).length, icon: Receipt },
              { label: 'Pending Payments', value: pendingInvoices.length, icon: Receipt },
            ].map((stat, idx) => (
              <div key={idx} className="bg-[#121212] rounded-2xl border border-white/5 p-6 hover:border-white/10 transition-colors">
                <div className="flex items-center justify-between mb-3">
                  <stat.icon className="w-5 h-5 text-white/20" />
                  <span className="text-[10px] text-white/30 font-sans uppercase tracking-wider">{stat.label}</span>
                </div>
          ...
          {pendingInvoices.length > 0 && (
            <div className="bg-[#121212] border border-white/5 rounded-2xl p-6 md:p-8">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xs font-bold uppercase tracking-widest text-white/80">Pending Payments</h2>
                  <p className="text-[10px] text-white/30 mt-1 font-sans">Total pending: ₦{totalPending.toLocaleString('en-NG')}</p>
                </div>
                <button
              ...
              <div className="space-y-3">
                {pendingInvoices.slice(0, 3).map((inv) => (
                  <div
                    key={inv.id}
                    className="flex items-center justify-between p-4 bg-[#0A0A0A] rounded-xl border border-white/5 hover:border-primary/20 transition-all"
                  >
                    <div>
                      <p className="text-sm font-bold text-white">{inv.projectName}</p>
                      <p className="text-[10px] text-white/30 mt-0.5 font-sans">{inv.id} — {inv.date}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <p className="text-base md:text-lg font-black text-primary font-display">₦{inv.amount.toLocaleString('en-NG')}</p>
                      <button
                        onClick={() => navigate('/billing')}
                        className="px-4 py-2 bg-primary hover:bg-primary-hover text-white rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all"
                      >
                        Pay
                      </button>
                    </div>
              ...
              {[
                {
                  ...
                  desc: 'Configure a project & generate instant invoice',
                  icon: Calculator,
